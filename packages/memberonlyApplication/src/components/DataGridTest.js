  ...
  },
  {
    ...
    renderCell: (params) => (
      <Stack spacing={1} direction='row' alignItems='center' sx={{ px: 1, width: 1, height: 1 }}>
        <LinearProgress value={params.row.performance} variant='determinate' color={(params.row.performance < 30 && 'error') || (params.row.performance > 30 && params.row.performance < 70 && 'warning') || 'primary'} sx={{ width: 1, height: 6 }} />
...
export default function DataGridCustom({ data }) {
  ...
  const getTogglableColumns = () => columns.filter((column) => !hiddenFields.includes(column.field)).map((column) => column.field)
