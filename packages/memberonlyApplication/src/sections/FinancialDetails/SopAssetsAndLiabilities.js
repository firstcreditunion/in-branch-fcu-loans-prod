...
import RHFSOPAsset from '../../forms/FinancialDetails/RHFSOPAsset'
import RHFSOPLiability from '../../forms/FinancialDetails/RHFSOPLiability'

//* MUI
import { Typography, Stack, Button, Divider, Chip, Switch } from '@mui/material'
...
const SopIncomeAndExpense = () => {
  ...
  const liabilityData = liabilityItems?.map((liabilityItem) => {
    if (liabilityItem?.amount1 != null || liabilityItem?.amount2 != null || liabilityItem?.amount3 != null || liabilityItem?.creditLimit != null || liabilityItem?.outStandingBalance != null) {
      return {
  ...
  const incomeData = incomeItems?.map((incomeItem) => {
    if (incomeItem?.amount1 != null || incomeItem?.amount2 != null || incomeItem?.amount3 != null) {
      return {
        ...
        amount1: incomeItem?.amount1 == null ? 0 : incomeItem?.amount1,
        amount2: incomeItem?.amount2 == null ? 0 : incomeItem?.amount2,
        amount3: incomeItem?.amount3 == null ? 0 : incomeItem?.amount3,
      }
    }
  })
  ...
  const expenseData = expenseItems?.map((expenseItem) => {
    if (expenseItem?.amount1 != null || expenseItem?.amount2 != null || expenseItem?.amount3 != null) {
      return {
        ...
        amount1: expenseItem?.amount1 == null ? 0 : expenseItem?.amount1,
        amount2: expenseItem?.amount2 == null ? 0 : expenseItem?.amount2,
        amount3: expenseItem?.amount3 == null ? 0 : expenseItem?.amount3,
      }
    }
  })

  const assets = assetData?.filter((data) => {
    return !(data == null)
  })

  const liabilities = liabilityData?.filter((data) => {
    return !(data == null)
  })
  ...
  useEffect(() => {
    ...
    if (assets?.length === 0 && liabilities?.length === 0 && income?.length === 0 && expense?.length === 0) return

    dispatch(getSopSummary(config))
  }, [assets, liabilities])
