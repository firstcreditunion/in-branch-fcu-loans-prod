...
function Passport() {
  ...
  const handlePassportNo = (event) => {
    dispatch(identificationActions.setPassportNo(event.target.value))
  }
  ...
  return (
    <Form>
      <Stack direction='column' spacing={3} justifyContent='flex-start' sx={{ pb: 5 }}>
        <InputField name='passportNo' label='Passport Number' type='text' control={control} errorInput={!!errors.passportNo} helperTextInput={errors?.passportNo?.message} onInputChange={handlePassportNo} hasTooltip={false} placeholder='Passport Number' />
