...
function JointKiwiAccessCard() {
  const kiwiAccessCardNo = useSelector((state) => state.identificationReducer.identificationTypes.kiwi_access_card.jointdata.kiwiAccessCardNo)
  const kiwiAccessCardIssueDate = useSelector((state) => state.identificationReducer.identificationTypes.kiwi_access_card.jointdata.kiwiAccessCardIssueDate)
  const defKiwiAccessCardIssueDate = useSelector((state) => state.identificationReducer.identificationTypes.kiwi_access_card.jointdata.defKiwiAccessCardIssueDate)
  const kiwiAccessCardExpiryDate = useSelector((state) => state.identificationReducer.identificationTypes.kiwi_access_card.jointdata.kiwiAccessCardExpiryDate)
  const defKiwiAccessCardExpiryDate = useSelector((state) => state.identificationReducer.identificationTypes.kiwi_access_card.jointdata.defKiwiAccessCardExpiryDate)
  const kiwiAccessCardPhotoUrl = useSelector((state) => state.identificationReducer.identificationTypes.kiwi_access_card.jointdata.kiwiAccessCardPhotoUrl)
  ...
  useEffect(() => {
    // console.log('Joint Kiwi Access Card Control: ', control)
    dispatch(identificationActions.setIsValidJointKiwiAccessCardDetails(isValid))

    if (isValid) {
      dispatch(identificationActions.setIsValidJointKiwiAccessCardCounter(1))
    } else {
      dispatch(identificationActions.setIsValidJointKiwiAccessCardCounter(0))
    }
  }, [isValid])
  ...
  const handleKiwiAccessCardNo = (event) => {
    dispatch(identificationActions.setJointKiwiAccessCardNo(event.target.value))
  }
  const handleKiwiAccessCardIssueDate = (date) => {
    dispatch(identificationActions.setJointKiwiAccessCardIssueDate(date))
  }
  const handleKiwiAccessCardExpiryDate = (date) => {
    dispatch(identificationActions.setJointKiwiAccessCardExpiryDate(date))
  }
  const handleKiwiAccessCardPhotoUrl = (file, preview) => {
    dispatch(identificationActions.setJointKiwiAccessCardPhotoUrl({ file, preview }))
  }
  ...
  return (
    <Form>
      <Stack direction='column' spacing={3} justifyContent='flex-start' sx={{ pb: 5 }}>
        <InputField name='kiwiAccessCardNo' label='Card Number' type='text' control={control} errorInput={!!errors.kiwiAccessCardNo} helperTextInput={errors?.kiwiAccessCardNo?.message} onInputChange={handleKiwiAccessCardNo} hasTooltip={false} />
        ...
        <DatePicker id='kiwiAccessCardExpiryDate' name='kiwiAccessCardExpiryDate' onDateChange={handleKiwiAccessCardExpiryDate} label='Expiry Date' control={control} variant='outlined' openTo='year' format='DD/MM/YYYY' date={kiwiAccessCardExpiryDate} minDate={defKiwiAccessCardExpiryDate} isRequired={true} />
        {/* <Box sx={{ mb: 5 }}>
          <UploadPhoto
            accept='image/*'
...
export default JointKiwiAccessCard
