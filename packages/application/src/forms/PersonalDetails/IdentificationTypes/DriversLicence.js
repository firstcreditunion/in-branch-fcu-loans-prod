...
function DriverLicence() {
  ...
  const licenceType = useSelector((state) => state.identificationReducer.identificationTypes.drivers_licence.data.licenceType)
  ...
  const defDrLicenceIssueDate = useSelector((state) => state.identificationReducer.identificationTypes.drivers_licence.data.defDrLicenceIssueDate)
  ...
  const defDrLicenceExpiryDate = useSelector((state) => state.identificationReducer.identificationTypes.drivers_licence.data.defDrLicenceExpiryDate)
  const drLicencePhotoUrl = useSelector((state) => state.identificationReducer.identificationTypes.drivers_licence.data.drLicencePhotoUrl)
  ...
  useEffect(() => {
    ...
    dispatch(identificationActions.setIsValidDrLicenceDetails(isValid))
    ...
    if (isValid) {
      ...
      if (secureSessionID) dispatch(identificationActions.setisValidSovDriversLicence(false))
      return
    }
  }, [isValid])
  ...
  useEffect(() => {
    ...
    dispatch(identificationActions.setdriversLicenceNo(''))
    dispatch(identificationActions.setdriversLicenceVersion(''))
    dispatch(identificationActions.setdrlicenceType(''))
    dispatch(identificationActions.setdrLicenceIssueDate(null))
    dispatch(identificationActions.setdrLicenceExpiryDate(null))
    dispatch(identificationActions.setdrLicencePhotoUrl({}))

    dispatch(identificationActions.setOnSubmitDrLicenceDetails(null))
    dispatch(identificationActions.setIsValidDrLicenceCounter(0))
  }, [checkedIdentificationTypes])

  function onSubmit() {
    console.log('Submitted DR Licence')
  }
  ...
  const handleLicenceType = (event) => {
    dispatch(identificationActions.setdrlicenceType(event.target.value))
  }
  ...
  const handleDrLicencePhotoUrl = (file, preview) => {
    dispatch(identificationActions.setdrLicencePhotoUrl({ file, preview }))
  }
    ...
    <Form>
      <Stack direction='column' spacing={3} justifyContent='flex-start' sx={{ pb: 5 }}>
        <RadioGroups id='licenceType' name='licenceType' onRadioChange={handleLicenceType} label='What type of drivers licence do you hold?' value={licenceType} control={control} variant='' defaultValue={licenceType || ''}>
          <FormControlLabel value='Learners' control={<Radio size='small' />} label='Learners' />
          <FormControlLabel value='Restricted' control={<Radio size='small' />} label='Restricted' />
